...
local DEFAULT_SESSION_NAME = "session@lua-resty-aws"
...
-- @tparam[opt='session@lua-resty-aws'] string opts.session_name session name
...
function TokenFileWebIdentityCredentials:refresh()
  if not self.sts then
    ...
    -- constructor is called when instaniating an AWS instance (creating a loop).
